...
# rebuild the vector index from the restored flat memory tiers:
skmemory reindex        # or: skcapstone doctor
```

Because the GFS backup deliberately omits the vector store and SQLite index,
**rebuild the index after restoring** — both reconstruct from the flat memory
tiers, which *are* in the archive.

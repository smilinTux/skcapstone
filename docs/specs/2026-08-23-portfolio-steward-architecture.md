...
**Date:** 2026-08-23
**Status:** REMEDIATION CANDIDATE AFTER FAILED REVIEW
**Decision owner:** Human owner
**Architecture card:** `b151ac5a`
**Parent epic:** `b43537a8`

Exact authority, fencing, review, profile, and handoff contracts are defined in
the [governance companion](2026-08-23-portfolio-steward-governance-contracts.md).
...
CardStore remains workflow truth; the card-gated fenced mutation service may
append the exact claim only after CapAuth authorization. AutoCoder and
SKHarness execute only the exact claimed leaf. An independent reviewer
...
Cross-role calls use the companion's `RoleInvocationV1`. The server derives
identity, target, and scope. A Steward profile is nonselectable and defaults to
zero tools plus explicit `portfolio.read` and `portfolio.propose` grants.
...
The coordination writer is the projection edge of the fenced authoritative
mutation service defined in the governance companion. It cannot call a model,
infer intent, select another card, or broaden the exact reserved operation.
...
Only `content` is authority-bearing. Persona, session, generated ids,
timestamps, objective text, and rendered prose live in `presentation` and do
not affect `plan_content_hash`. Canonicalization and exact field contracts are
defined in the governance companion.
...
The deterministic allocator reevaluates one recommendation against a fresh
snapshot and returns the companion's `AllocationDecisionV1`. It binds the plan
content hash, exact operation, card and dependency revisions, approval, policy,
capacity, WIP, lease generation, executor, repo revision, expiry, and
idempotency key. Its authorization state remains `pending`.

The writer rejects an ineligible, expired, used, stale, mismatched, forced, or
wrong-executor decision.
...
The fenced mutation service obtains final CapAuth authorization immediately
before its atomic reservation. It binds all current preconditions, the decision
and obligations, writer epoch, lease generation, reservation, outbox, and
readback receipt. Denial or uncertainty creates no executable claim.
...
- A claim has an explicit reservation, lease generation, duration, and owner.
- Lease renewal requires a current writer fence, heartbeat, and authorization.
...
| `69a6bd7b` | SKPM-00R | First independent review, preserved as FAIL | SKPM-00 |
| `24d5b867` | SKPM-00F1 | Remediate failed findings F1 through F7 | SKPM-00 |
| `abab982e` | SKPM-00R2 | Independent review of remediated exact hashes | SKPM-00F1 |
| `215edee5` | SKPM-00H | Exact-hash human architecture approval | SKPM-00R2 PASS |
| `7efc76c0` | SKPM-01A | Pure contracts, readiness, ordering, and review types in `skcoord` | SKPM-00R2 PASS |
...
| `226430d5` | SKPM-MUT-01 | Fenced atomic managed-claim primitive in SKCoord | SKPM-00H, SKPM-01R |
| `048c5de2` | SKPM-MUT-02 | Authoritative fenced mutation service | SKPM-MUT-01 |
| `7e72aca6` | SKPM-MUT-02R | Independent concurrency and recovery qualification | SKPM-MUT-02 |
| `a0a89b24` | SKPM-PROF-01 | Common fail-closed service-profile registry | SKPM-00H |
| `b1ce03c4` | SKPM-PROF-02 | SKMemory ownership and fallback enforcement | SKPM-PROF-01 |
| `248aa691` | SKPM-PROF-03 | Jarvis Mission Control service filtering | SKPM-PROF-01 |
| `90ebc6a7` | SKPM-PROF-R | Cross-repo profile qualification | SKPM-PROF-01, SKPM-PROF-02, SKPM-PROF-03 |
| `47b32de2` | SKPM-HO-00 | Canonical editable SKHarness checkout | SKPM-00H |
| `8fb27a78` | SKPM-ATL-01 | Explicit Atlas Portfolio capability-deny policy | SKPM-00H, SKPM-MUT-02 |
| `62406541` | SKPM-ATL-R | Independent Atlas negative-authorization proof | SKPM-ATL-01, SKPM-MUT-02R |
| `d5c6f539` | SKPM-02 | Governed shadow Steward proposal service | SKPM-00H, SKPM-01R, profile and Atlas reviews, existing SKCP gates |
| `2850e05b` | SKPM-03 | Deterministic allocator in simulation | SKPM-00H, SKPM-01R, mutation and Atlas reviews, existing SKCP gates |
| `de712b36` | SKPM-HO-01 | Authenticated SKCapstone handoff producer | SKPM-03, mutation review, profile review |
| `113aeadd` | SKPM-HO-02 | Exact SKHarness handoff consumer | SKPM-HO-00, SKPM-HO-01 |
| `fe3877e5` | SKPM-HO-R | Producer and consumer conformance review | SKPM-HO-01, SKPM-HO-02 |
| `a981f7bd` | SKPM-04 | Persona-neutral front door and exact-card handoff | SKPM-02, SKPM-03, mutation, profile, handoff, and Atlas reviews |
...
SKPM-01A resumes only after SKPM-00R2 returns PASS. It remains pure logic over
public synthetic fixtures with no live board read or mutation. The

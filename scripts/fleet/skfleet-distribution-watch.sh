...
sample() {
  local host out workers result pool ready total_workers=0 total_ready=0 unavailable=0 details=""
  ...
  local queue queue_active queue_queued now current previous
  ...
  printf '%s|state=%s|workers=%d|workable=%d|unavailable_hosts=%d|queue_active=%s|queue_queued=%s|%s\n' \
    "$now" "$current" "$total_workers" "$total_ready" "$unavailable" "$queue_active" "$queue_queued" "$details" >> "$log_dir/watch.log"

  if [[ "$current" != "$previous" ]]; then
    if [[ "$current" == zero || "$current" == unavailable ]]; then
      skmail send jarvis lumina urgent FLEET-DISTRIBUTION-DOWN \
        "Fleet watcher transition at $now: state=$current workers=$total_workers workable_cards=$total_ready unavailable_hosts=$unavailable queue_active=$queue_active queue_queued=$queue_queued. Details: $details. No restart, rotation trigger, gateway mutation, or mailbox acknowledgment was performed." >/dev/null
    elif [[ -n "$previous" ]]; then
      skmail send jarvis lumina normal FLEET-DISTRIBUTION-RECOVERED \
        "Fleet watcher recovery at $now: workers=$total_workers workable_cards=$total_ready unavailable_hosts=$unavailable queue_active=$queue_active queue_queued=$queue_queued. Details: $details." >/dev/null
    fi
    printf '%s\n' "$current" > "$state_file"
  fi
  printf 'state=%s workers=%d workable=%d unavailable_hosts=%d queue_active=%s queue_queued=%s\n' \
    "$current" "$total_workers" "$total_ready" "$unavailable" "$queue_active" "$queue_queued"
}

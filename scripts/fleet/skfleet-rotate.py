...
def reap_dead_claims():
    ...
    # Collect all failed conditions for truthful diagnostic. The decision is
    # fail-closed: ALL conditions must pass before reaping proceeds.
    failed = []
    if not oldest:
        failed.append("no_fresh_reports")
    if nhosts < REAP_QUORUM:
        failed.append("insufficient_quorum:_%d_reporting_hosts_needed_%d" % (nhosts, REAP_QUORUM))
    if nhosts < known:
        failed.append("insufficient_fresh_coverage:_%d_reporting_hosts_vs_%d_known_hosts" % (nhosts, known))
    
    if failed:
        log(d, "REAP|%s|blocked: %s; reaped nothing" % (HOST, ", ".join(failed)))
        return 0
    freed = 0

...
# Load lane affinity enforcement module
import sys
_lane_module_path=os.path.join(os.path.dirname(os.path.abspath(__file__)),"lanes.py")
if os.path.exists(_lane_module_path):
    sys.path.insert(0, os.path.dirname(_lane_module_path))
    try:
        from lanes import lane_affinity, count_compatible_slots, filter_compatible_lanes, lane_preference_order
        _LANE_AFFINITY_AVAILABLE=True
    except Exception as _e:
        print("  WARN lane affinity module unavailable (%s): label-based lane constraints disabled"%_e)
        _LANE_AFFINITY_AVAILABLE=False
else:
    _LANE_AFFINITY_AVAILABLE=False
for _L in LANES:
...
_PINNED_IDS=set()
pool=[]; blocked=0; foreign_skipped=0; skipped_unclaimable=0; skipped_terminal=0; skipped_blocked=0; pinned_elsewhere=0; skipped_lane_incompatible=0
for cd in sorted(glob.glob(CARDS+"/*")):
    ...
    if _pin == HOST:
        _PINNED_IDS.add(cid)
    # ---- lane affinity enforcement -------------------------------------------
    # Cards with lane-only labels (codex-only, glm-only, escalation-required) may
    # only use their designated lane. Cards without these labels use the default
    # preference order (GLM first, then Codex). Cards with conflicting lane labels
    # are excluded from the pool with a log entry.
    #
    # This fixes the live defect where cards 12eaed95 and ac8592fc carried
    # codex-only yet were claimed and launched as glm-auto workers.
    if _LANE_AFFINITY_AVAILABLE:
        try:
            _card_lane = lane_affinity(labels)
            _compatible_slots = count_compatible_slots(labels, LANES)
            if _card_lane is not None and _compatible_slots == 0:
                skipped_lane_incompatible += 1
                log(d, "SKIPPED_LANE_INCOMPATIBLE|%s|%s|lane=%s|no free slots" % (HOST, cid, _card_lane))
                continue
        except ValueError as _ve:
            # Conflicting lane-only labels
            skipped_lane_incompatible += 1
            log(d, "SKIPPED_LANE_CONFLICT|%s|%s|%s" % (HOST, cid, str(_ve)))
            continue
    ...
    # Append with labels for lane compatibility checks during selection
...
log(d,"POOL|%s|ready=%d sklegal=%d eng=%d biz=%d dep_blocked=%d unclaimable=%d itil_closed=%d blocked_backoff=%d pinned_elsewhere=%d foreign=%d lane_incompatible=%d top_unblocks=%d"
      %(HOST,len(pool),lc[0],lc[1],lc[2],blocked,skipped_unclaimable,skipped_terminal,skipped_blocked,pinned_elsewhere,foreign_skipped,skipped_lane_incompatible,top))
...
# ---- lane-aware card selection -----------------------------------------
# For each card, filter lanes to those compatible with its labels, then select
# the first available compatible lane. This enforces codex-only, glm-only, and
# escalation-required constraints while preserving the default preference order
# for unlabeled cards.
picks=[]; _i=0
...
while _i<len(owned) and len(picks)<MAX_LAUNCH:
    progressed=False
    _card = owned[_i]
    _cid = _card[2]  # card ID is at index 2
    _labels = _card[3].get("labels", []) if hasattr(_card[3], 'get') else []
    
    # Determine compatible lanes for this card
    if _LANE_AFFINITY_AVAILABLE:
        try:
            _compatible_lanes = filter_compatible_lanes(_labels, LANES)
            # For unlabeled cards, use preference order (GLM first)
            if lane_affinity(_labels) is None:
                _compatible_lanes = lane_preference_order(_compatible_lanes)
        except ValueError as _ve:
            # Should have been caught during pool building, but skip to be safe
            log(d, "SKIPPED_LANE_CONFLICT|%s|%s|%s" % (HOST, _cid, str(_ve)))
            _i += 1
            continue
    else:
        _compatible_lanes = sorted(LANES, key=lambda lane: 0 if lane["name"] == "glm" else 1)
    
    # Try each compatible lane in order
    for lane in _compatible_lanes:
        if remaining[lane["name"]] <= 0:
            continue
        picks.append((lane, _card))
        _i += 1
        remaining[lane["name"]] -= 1
        progressed = True
        
        # Log the lane choice for debugging
        _card_lane = lane_affinity(_labels) if _LANE_AFFINITY_AVAILABLE else None
        if _card_lane:
            log(d, "LANE_AFFINITY|%s|%s|lane=%s|label=%s" % (HOST, _cid, lane["name"], _card_lane))
        
        if _i >= len(owned) or len(picks) >= MAX_LAUNCH:
            break
    
    if not progressed:
        # Card has no compatible free slots - stop, do not consume another lane
        log(d, "NO_COMPATIBLE_SLOT|%s|%s" % (HOST, _cid))
        break
if not picks:
...
for _LANE,(_,_,cid,core,labels,_nb) in picks:

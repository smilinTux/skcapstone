...
for _LANE,(_,_,cid,core,_labels,_nb) in picks:
    ...
    _RAILS=("CONSTRAINTS (standing rails, non-negotiable):\n"
      ...
      "\n"
      "HOW TO RECORD A VERDICT. Mailing a verdict is NOT recording one. The board\n"
      "reads the CardStore, not your mail. Run all three, substituting your own\n"
      "agent name for <you> and the real card id for <card>:\n"
      "    skcapstone coord link <card> outcome \"<PASS|PASS_FOR_REVIEW|BLOCKED>|<detail>\" --agent <you>\n"
      "    skcapstone coord link <card> evidence_sha256 <sha256 of your evidence file> --agent <you>\n"
      "    skcapstone coord complete <card> --agent <you>\n"
      "  coord link takes THREE positional arguments: card, key, value. There is no\n"
      "  --key or --value form; guessing one fails with a usage error.\n"
      "\n"
      "THEN VERIFY IT LANDED. The command prints a confirmation even when it stores\n"
      "an EMPTY value, so its output does not prove anything. Measured 2026-08-31:\n"
      "3,047 of 22,843 evidence events board-wide, 13 percent, carry a null key AND\n"
      "a null value across 1,054 cards. Read your verdict back before you finish:\n"
      "    grep -h <card> ~/.skcapstone/coordination/card_events/*.jsonl | tail -3\n"
      "If your outcome is not in that output, it was not recorded and the card will\n"
      "be dispatched again to redo work you already did.\n"
      "\n"
      "WHY THIS IS SPELLED OUT. Measured over 12 hours: cards claimed by codex\n"
      "recorded evidence 100 percent of the time, glm 66 percent, qwen 39 percent,\n"
      "on the same fleet with the same command. Reviews were performed correctly and\n"
      "then reported only by mail, leaving the parent parked and the reviewer own\n"
      "card open. The work was done. The board could not see it.\n"
      "\n"

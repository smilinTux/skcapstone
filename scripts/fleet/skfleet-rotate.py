...
Fixes review host distinctness (card 4896345b):
  3. Review cards are no longer assigned to workers on the same physical host
     as their preparer. The preparer host is determined from the durable launch
     record (the 'node' field in the claim event), not from the worker agent name.
     This prevents assigning a review to a worker on the same machine as the
     preparer, which would violate independence requirements. The count of cards
     deferred for host distinctness is logged in the POOL message.
"""
...
# ---- review host distinctness ----------------------------------------------
# Review cards require a physically distinct host from the preparer to ensure
# independence. The preparer host is determined from the durable launch record
# (the 'node' field in the claim event), NOT from the worker agent name.
# This prevents assigning a review to a worker on the same physical machine as
# the preparer, which would violate independence requirements.
_preparer_host_cache = {}

def _is_review_card(core):
    """True if this card is a review card based on its title."""
    title = str(core.get("title") or "")
    return "[REVIEW]" in title

def _get_preparer_host(review_cid):
    """Get the physical host of the preparer for a review card.
    
    For review cards, the first dependency is the preparer card. We read the
    preparer's claim event to find the 'node' field, which records the physical
    host where the work actually occurred.
    
    Returns (host, source) where:
      - host is the physical host name or None if not determinable
      - source describes where the information came from for logging
    
    This degrades safely: if the preparer host cannot be determined, the card
    remains assignable to avoid stranding review work permanently.
    """
    global _preparer_host_cache
    if review_cid in _preparer_host_cache:
        return _preparer_host_cache[review_cid]
    
    # Get the review card's core to find its dependencies
    core_path = os.path.join(CARDS, review_cid, "core.json")
    try:
        core = json.load(open(core_path))
    except Exception:
        _preparer_host_cache[review_cid] = (None, "core_not_found")
        return (None, "core_not_found")
    
    # Get the preparer card ID (first dependency for review cards)
    deps = folded_dependencies(review_cid, core)
    if not deps:
        _preparer_host_cache[review_cid] = (None, "no_dependencies")
        return (None, "no_dependencies")
    
    preparer_cid = deps[0]
    
    # Read the preparer's events to find the claim with node info
    try:
        events = event_rows(preparer_cid)
    except Exception:
        _preparer_host_cache[review_cid] = (None, "events_read_failed")
        return (None, "events_read_failed")
    
    # Find the LAST claim event, which represents where the work was actually completed.
    # A card may have multiple claims (retries, worker movement), but the last one
    # before completion is the physical host where the preparer work occurred.
    last_claim_node = None
    for event in events:
        if event.get("action") == "claim":
            node = event.get("node")
            if node:
                last_claim_node = node
    
    if last_claim_node:
        if last_claim_node in ROTATION_HOSTS:
            _preparer_host_cache[review_cid] = (last_claim_node, "last_claim_event")
            return (last_claim_node, "last_claim_event")
        else:
            # Node exists but is not a rotation host - still valid
            _preparer_host_cache[review_cid] = (last_claim_node, "last_claim_event_nonrotation")
            return (last_claim_node, "last_claim_event_nonrotation")
    
    # No claim event found or no node field
    _preparer_host_cache[review_cid] = (None, "no_claim_with_node")
    return (None, "no_claim_with_node")
...
_PINNED_IDS=set()
pool=[]; blocked=0; foreign_skipped=0; skipped_unclaimable=0; skipped_terminal=0; skipped_blocked=0; pinned_elsewhere=0; skipped_review_host_distinctness=0
for cd in sorted(glob.glob(CARDS+"/*")):
    ...
    # ---- review host distinctness check ----------------------------------
    # For review cards, ensure they are not assigned to the same physical host
    # as the preparer. The preparer host is read from the durable launch record.
    if _is_review_card(core):
        preparer_host, source = _get_preparer_host(cid)
        if preparer_host and preparer_host == HOST:
            # The preparer ran on this host, so we cannot take this review.
            # Skip it now, and if no other host can take it this tick, it will
            # be left in the pool for the next tick.
...
log(d,"POOL|%s|ready=%d sklegal=%d eng=%d biz=%d dep_blocked=%d unclaimable=%d itil_closed=%d blocked_backoff=%d pinned_elsewhere=%d review_host_distinctness=%d foreign=%d top_unblocks=%d"
      %(HOST,len(pool),lc[0],lc[1],lc[2],blocked,skipped_unclaimable,skipped_terminal,skipped_blocked,pinned_elsewhere,skipped_review_host_distinctness,foreign_skipped,top))
...
for _LANE,(_,_,cid,core,_nb) in picks:
    ...
    brief=("Work only SKCapstone card %s. Claim it first with a supported command. "
      ...
      "CARD %s (%s)\nTITLE: %s\nDESCRIPTION: %s\n\nACCEPTANCE CRITERIA:\n%s\n\n"
      "CONSTRAINTS (NIGHT-HANDOFF, non-negotiable):\n"
      ...
      "- No commit or push to origin/main, merge, deploy, restart, live gateway or config mutation, credential disclosure, cleanup, WAKE-02 enablement, live_execution, or automerge.\n"
    ...
    # For review cards, record the preparer host to show we checked distinctness
    launch_info = "lane=%s" % _LANE["name"]
    if _is_review_card(core):
        preparer_host, source = _get_preparer_host(cid)
        if preparer_host:
            launch_info += "|preparer_host=%s|preparer_host_source=%s" % (preparer_host, source)
        else:
            launch_info += "|preparer_host=unknown|preparer_host_source=%s" % source
    log(d,"%s|%s|%s|%s|%s"%("LAUNCHED" if ok else "LAUNCH_FAILED",HOST,sess,cid,launch_info))
    time.sleep(2)

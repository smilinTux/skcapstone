...
for _LANE,(_,_,cid,core,_labels,_nb) in picks:
    ...
    brief=_RAILS + ("WORKER AGENT BEAT CONTRACT\n"
      "Report phase and step on entering each phase and immediately when blocked. Use dispositions "
      "RUNNING, WAITING_DEPENDENCY, BLOCKED_NEEDS_HUMAN, or DEGRADED_RETRYING. "
      "progress.token is an opaque progress token compared only for equality. A silent worker is indistinguishable from a hung worker. "
      "Non-RUNNING states emit a one-line skmail event to heartbeat; routine RUNNING beats are state, through skmail only for dispositions. "
      "Mid-run emission is not proven: MID_RUN_EMISSION_UNPROVEN_LAUNCHER_BLOCKS_SUBPROCESS_RUN.\n\n"

...
CONFLICT_RE = re.compile(r"^(?P<base>.+?)\.sync-conflict-.+$")


def canonical_path(path: Path) -> Path:
    """Return the canonical sibling represented by a Syncthing conflict."""
    match = CONFLICT_RE.match(path.name)
    if not match:
        return path
    # Agent projections are JSON; Syncthing places the conflict marker before
    # the extension, so restore the canonical .json sibling without reading bytes.
    return path.with_name(f"{match.group('base')}.json")


def first_seen(path: Path) -> str:
    """Use immutable filesystem birth time when available, mtime otherwise."""
    stat = path.stat()
    value = getattr(stat, "st_birthtime", stat.st_mtime)
...
def classify(path: Path, *, now: datetime, stale_after: int) -> dict[str, Any]:
    ...
    # This branch intentionally precedes read_text/json parsing. Conflict bytes
    # are evidence only and can never affect capacity, ownership, or health.
    if CONFLICT in name:
        result.update(
            disposition="conflict-copy",
            canonical_path=str(canonical_path(path)),
            first_seen=first_seen(path),
        )
        return result
    try:
...
def build_report(root: Path, *, now: datetime, stale_after: int) -> dict[str, Any]:
    ...
    return {
        ...
        # Capacity is explicitly the canonical projection population.
        "capacity": counts.get("canonical", 0) + counts.get("stale", 0),
        "records": records,
    }

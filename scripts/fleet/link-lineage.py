...
def _reviewer_candidates(home: Path, explicit: list[dict[str, Any]] | None) -> list[dict[str, str]]:
    ...
    for candidate in value:
        ...
        identity, host, session, workspace = (candidate.get(k) for k in ("identity", "host", "session", "workspace"))
        if not all(isinstance(v, str) and v.strip() for v in (name, seat, fingerprint, identity, host, session, workspace)):
            raise ValueError("reviewer identity fields are required")
        if seat.strip().lower() != "seraph" or name.strip().lower() != "seraph" or identity.strip().lower().startswith("jarvis") or not _SHA256.fullmatch(fingerprint.strip()):
...
def _revision(home: Path, card_id: str, card: dict[str, Any] | None = None) -> str | None:
    ...
    if card is not None:
        stable = {
            ...
            "verdict": str((card.get("links") or {}).get("verdict") or (card.get("links") or {}).get("outcome") or "").strip().upper(),
        }
...
def _mapping_hash(repository: str, number: int, head: object, base: object, source: dict[str, Any], review: dict[str, Any], source_generation: str, review_revision: str, review_verdict: str) -> str:
    value = {
...
def reconcile(open_prs: list[dict[str, Any]], cards: list[dict[str, Any]], home: Path, exclusions: dict[str, str] | None = None, reviewer_candidates: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    for pr in sorted(open_prs, key=lambda x: (str(x.get("repository", "")), int(x["number"]))):
        ...
        referenced = [cards_by_id[card_id] for card_id in CARD_RE.findall(pr_text) if card_id in cards_by_id]
        candidates = []
        ...
        def is_review(card: dict[str, Any]) -> bool:
            ...
            return "review" in card_labels or "[REVIEW]" in title or any(label.endswith("-review") for label in card_labels)
        ...
        reviews = [c for c in cards if is_review(c) and len(sources) == 1 and f"parent-{sources[0]['id']}" in labels(c)]
        ...
        bound_terminal_reviews = [
            ...
            if _review_is_bound_to_pr(
                review, repository, number, pr.get("headRefOid")
            )
        ]
        selected_reviews = (
            bound_terminal_reviews
            if len(bound_terminal_reviews) == 1
            else terminal_reviews
        )
        if len(sources) != 1:
            ...
            review_verdict = str(review_links.get("verdict") or review_links.get("outcome") or "").strip().upper()
            if source_generation and review_revision:
                records[f"{repository}#{number}"] = {
                    ...
                    "mapping_evidence_sha256": _mapping_hash(repository, number, pr.get("headRefOid"), pr.get("baseRefOid"), source, review, source_generation, review_revision, review_verdict),
                }
                continue
        diagnostics.append({"pr": number, "classification": classification, "candidate_source_cards": [c["id"] for c in sources], "candidate_review_cards": [c["id"] for c in reviews], "terminal_review_cards": [c["id"] for c in terminal_reviews], "head_bound_review_cards": [c["id"] for c in bound_terminal_reviews]})
    ...
    body = {"schema": "skfleet.link-lineage/v1", "source_revision": hashlib.sha256(_json(ordered_prs).encode()).hexdigest(), "coverage": counts, "unresolved_prs": sorted(r["pr"] for r in diagnostics if r["classification"] == "unresolved"), "records": records, "reviewer_candidates": candidates_from_authority, "diagnostics": diagnostics}

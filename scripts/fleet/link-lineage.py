...
def _terminal_review_verdict(value: object) -> str | None:
    """Return the typed terminal token without discarding its explanation."""
...
def _source_link_matches(card: dict[str, Any], repository: str, number: int, head: object) -> bool:
...
def _validate_exclusions(
    ...
    for record in records:
        ...
        if not isinstance(classification, str) or classification not in _EXCLUSION_CLASSIFICATIONS:
    ...
    for pr in sorted(open_prs, key=lambda x: (str(x.get("repository", "")), int(x["number"]))):
            ...
            diagnostics.append(
                {
                    "pr": number,
                    "repository": repository,
                    "head_revision": str(pr.get("headRefOid") or pr.get("head_sha") or ""),
                    "base_revision": str(pr.get("baseRefOid") or pr.get("base_sha") or ""),
        ...
        def is_review(card: dict[str, Any]) -> bool:
            ...
            return (
                ...
                or "[REVIEW]" in title
                or "[REREVIEW]" in title
                or "seat-seraph" in card_labels
        ...
        candidate_sources = [c for c in candidates if not is_review(c)]
        # A unique exact PR and full-head binding is stronger than historical
        # prose references. Multiple exact bindings remain ambiguous and fail
        # closed; with no exact binding, retain the legacy candidate behavior.
        sources = linked_sources if len(linked_sources) == 1 else candidate_sources
        reviews = [
            c
            for c in cards

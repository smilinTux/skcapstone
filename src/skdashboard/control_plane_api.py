...
def routes(
    ...
    *,
    ...
    async def schedule_forecasts(request):
        ...
        ready = result.get("state") == "ready" and result.get("abstention_reason") is None and all(type(value) is int for value in quantiles.values()) and quantiles["p50"] <= quantiles["p85"] <= quantiles["p95"] and (result.get("milestone_confidence") is None or isinstance(result.get("milestone_confidence"), float) and 0 <= result["milestone_confidence"] <= 1)

    ...
    *,
    ...
    for raw in raw_rows:
        ...
        if metadata["head_revision"] != head or metadata["base_revision"] != base:
            missing.append(key)
            continue
        if not _GIT_SHA.fullmatch(str(metadata["head_revision"])) or not _GIT_SHA.fullmatch(str(metadata["base_revision"])):
            missing.append(key)
            continue
        expected_mapping = _mapping_evidence(
            str(raw["repository"]), number, head, base,
            str(metadata["source_card"]), str(metadata["card_generation"]),
            str(metadata["review_card_id"]), str(metadata["review_card_revision"]), str(metadata["review_verdict"]),
        )

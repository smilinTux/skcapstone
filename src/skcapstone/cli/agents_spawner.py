...
def register_agents_spawner_commands(agents: click.Group) -> None:
    ...
    def agents_spawn(
        ...
        if baby:
            from ..baby_agents import BABY_AGENTS, get_baby_agent
    ...
    def agents_spawned(home: str):
        ...
        for r in results:
            ...
            status_style = (
                ...
                else "yellow" if status_val == "pending" else "red"
            )
            table.add_row(
                r.agent_name,
        ...
        console.print(
            Panel(
                table, title=f"[bold]Spawned Sub-Agents ({len(results)})[/]", border_style="cyan"
            )
        )

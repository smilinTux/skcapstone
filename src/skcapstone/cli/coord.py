...
def register_coord_commands(main: click.Group) -> None:
    ...
    def coord_claim(task_id, home, agent, casey_authorization, casey_change_id, force):
        ...
        try:
            ...
            # Fail fast here, then rerun this exact shared gate from _claim_task
            # after Board owns the card lock and before it mutates either
            # projection. The second check closes verdict/precheck races.
            assert_claim_admitted(home_path, task_id, agent)
            ag = claim_task_with_locked_admission(
                board,
                agent,
                task_id,
                force=force,

...
def register_coord_commands(main: click.Group) -> None:
    ...
    def coord_gates(task_id, home):
        ...
        from ..scheduler_decision import folded_revision
        from skcoord.scheduler_truth import evaluate_scheduler_truth
        ...
        try:
            ...
            if facts is None:
                raise click.ClickException(f"unknown card: {task_id}")
            eligible = truth.ready_count == 1
            primary_reason = "ready" if eligible else next(iter(truth.reason_counts))
        ...
        result = {
            ...
            "blocking_reasons": [] if eligible else [primary_reason],
        }

...
def systemd_service_identity(host: str, scope: str, unit_id: str) -> str:
    """Preserve legacy service IDs and hash only collision-prone identities."""

    unit, kind = unit_id.rsplit(".", 1)
    identity = unit if kind == "service" else f"{host}:{scope}:{unit_id}"
    slug = re.sub(r"[^\w.-]+", "-", identity.lower()).strip("-")
    if len(slug) <= 48:
        return identity
...
def _declared_service_id(host: str, unit: str) -> str:
    identity = systemd_service_identity(host, "system", unit)
    return make_ci_id(CIType.SERVICE.value, identity)


def _validated_profile(
    profile: object,
    profile_ids: set[str],
    endpoints: set[tuple[str, str, int]],
    owned_units: set[str],
) -> tuple:
    ...
    endpoint = (host, protocol, port)
    if endpoint in endpoints:
        raise ValueError("SKGateway ingress endpoint is already owned")
    raw_binds = profile.get("binds")
    if not isinstance(raw_binds, list) or not 3 <= len(raw_binds) <= _MAX_BINDS:
        raise ValueError("SKGateway ingress bind count is invalid")
    binds = [_validated_bind(item) for item in raw_binds]
    if len({address for address, _ in binds}) != len(binds):
        raise ValueError("SKGateway ingress bind addresses must be unique")
    ...
    for item in units:
        ...
        if (
            ...
            or (role == "socket") != unit.endswith(".socket")
    ...
    if len(known_units) != len(unit_rows):
        raise ValueError("SKGateway ingress unit ownership is ambiguous")
    ...
    roles_by_unit = {unit: role for role, (unit, _) in unit_rows.items()}
    for role, (unit, targets) in unit_rows.items():
        target_roles = tuple(roles_by_unit[target] for target in targets)
        if unit in targets or target_roles != _ROLE_TARGETS[role]:
            raise ValueError("SKGateway ingress role relationship is invalid")
    if known_units & owned_units:
        raise ValueError("SKGateway ingress unit is already owned")
    profile_ids.add(profile_id)
    endpoints.add(endpoint)
    owned_units.update(known_units)
...
def _profile_cis(profile: tuple) -> list[DiscoveredCI]:
    ...
    for role, (unit, targets) in sorted(unit_rows.items()):
        ...
        relationships = [("runs_on", make_ci_id(CIType.HOST.value, host))]
        if role == "socket":
            relationships.append(("binds", make_ci_id(CIType.PORT.value, f"{host}:{port}")))
...
def collect_declared_skgateway_ingress(home: Path) -> list[DiscoveredCI]:
    ...
    validated = [
        _validated_profile(profile, profile_ids, endpoints, owned_units) for profile in profiles
    ]

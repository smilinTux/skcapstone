...
class RevisionFencedReviewHandoff:
    """Advisory launch input. Only activated Niobe may validate and execute it."""
    ...
    @property
    def computed_recommendation_id(self) -> str:
        """Bind every field consumed by the handoff actuator."""
        ...
        return _digest(
            {
                ...
                "review_card_revision": self.review_card_revision,
                "reviewer": self.reviewer,
                "reviewer_identity": asdict(self.reviewer_identity),
...
def recommend_one_reviewer(
    ...
    *,
    ...
    handoff = RevisionFencedReviewHandoff(
        ...
        reviewer_identity=selected,
        observation_revision=revision,
    )
    return replace(handoff, recommendation_id=handoff.computed_recommendation_id)
...
def validate_handoff(
    ...
    *,
    ...
    current_reviewer_identity: ReviewerIdentity | None,
    active_review_card: str | None,
    used_recommendation_ids: set[str],
    actor: str,
) -> None:
    """Niobe-side stale/replay fence before the existing claim and launcher."""

    if actor.strip().lower() != "niobe":
        raise BoundaryError("only niobe may validate a review handoff")
    if handoff.recommender.strip().lower() != "link":
        raise BoundaryError("handoff recommender changed")
    if current_reviewer_identity is None:
        raise BoundaryError("current reviewer identity is required")
    if handoff.reviewer_identity.name.strip() != handoff.reviewer:
        raise BoundaryError("handoff reviewer name and identity changed")
    if handoff.reviewer_identity != current_reviewer_identity:
        raise BoundaryError("reviewer identity changed")
    if handoff.recommendation_id != handoff.computed_recommendation_id:
        raise BoundaryError("handoff recommendation digest changed")
    ...
    if current_observation.repository != handoff.repository:
        raise BoundaryError("PR repository changed")
    if current_observation.number != handoff.pr_number:
        raise BoundaryError("PR number changed")
    if current_observation.base_sha != handoff.base_sha:
        raise BoundaryError("PR base changed")
    if current_observation.source_card != handoff.source_card:
        raise BoundaryError("source card changed")

...
def test_five_host_candidate_inventory_counts_unique_ids(tmp_path: Path) -> None:
    """Repeated per-host pools count each eligible task exactly once."""
    pools = [
        "POOL|chiap01|ready=2 ids=aaaaaaaa,bbbbbbbb",
        "POOL|chiap02|ready=2 ids=aaaaaaaa,bbbbbbbb",
        "POOL|chiap03|ready=2 ids=bbbbbbbb,cccccccc",
        "POOL|chiap04|ready=1 ids=cccccccc",
        "POOL|chiap08|ready=3 ids=aaaaaaaa,bbbbbbbb,cccccccc",
    ]
    script = f"""
set -euo pipefail
...
for pool in "$@"; do record_candidate_pool "$pool"; done
printf '%d|%d\n' "${{#candidate_ids[@]}}" "$candidate_manifests_missing"
"""
    result = subprocess.run(
        ["bash", "-c", script, "inventory-test", *pools],
        check=True,
        capture_output=True,
        text=True,
...
def test_legacy_pool_is_reported_missing_without_double_counting(
    ...
    """A pool without an ID manifest is visible and never added by ready count."""
    script = f"""
set -euo pipefail
export SKFLEET_DISTRIBUTION_WATCH_LIB_ONLY=1
source {WATCH}
reset_candidate_inventory
record_candidate_pool 'POOL|chiap01|ready=9'
printf '%d|%d\n' "${{#candidate_ids[@]}}" "$candidate_manifests_missing"
"""
    result = subprocess.run(
        ["bash", "-c", script],

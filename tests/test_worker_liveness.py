...
def obs(**kw):
    base = dict(owner="agent", card_id="card", claim_generation="gen", process_identity="pid", managed_session=True, live_children=0, heartbeat_at=NOW - timedelta(hours=2), terminal_marker=None, skmail_response_at=None, workspace_recoverable=True, workspace_custody="workspace")
    base.update(kw)
...
def test_metrics_are_deterministic():
    rows = [classify(obs(), now=NOW), classify(obs(quiet_tool_wait=True), now=NOW), classify(obs(terminal_marker="PASS", live_children=2), now=NOW)]
    m = metrics(rows, assistance_latencies=[2, 4], retirement_latencies=[3], preserved_work=[True, False])

...
def test_rejects_unexpanded_host_identity(tmp_path: Path) -> None:
    for host in ("SKAGENT", "$SKAGENT", "${SKAGENT}", "AGENT", "YOUR_NAME"):
        try:
            MODULE.append(tmp_path, "worker", "jarvis", "normal", "x", "y", host)
        except ValueError:
            continue
        raise AssertionError(host)


def test_append_corrects_legacy_mailbox_mode(tmp_path: Path) -> None:
    ...
    assert box.stat().st_mode & 0o777 == 0o600

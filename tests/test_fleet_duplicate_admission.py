...
def _load_namespace() -> dict:
    ...
    body = [
        ...
        or (isinstance(node, ast.Assign)
            and any(isinstance(target, ast.Name) and target.id in _ASSIGN_NAMES
...
def test_concurrent_launchers_exactly_one_admission(tmp_path) -> None:
    ...
    def contender(index: int) -> None:
        ...
        result = _acquire(
            namespace, tmp_path, "c32a1001",
            "pi-codex-chiap08-c32a1001-manual" if index == 0
    ...
    assert all(
        r["reason"] == "live-holder" for r in results if not r["admitted"]
    ), results
...
def test_84baffed_shape_unit_holder_blocks_then_self_release_recovers(
    ...
    duplicate = _acquire(
        namespace, tmp_path, "84baffed", "pi-codex-chiap01-84baffed",
        "f9333eedd5fd4f248e7204c61cc90eca",
        "/home/skuser01/work/skcapstone-84baffed",
        unit=unit, session="codex-auto-84baffed",
        pid=_dead_pid(), runner=runner,
    )
    ...
    refused = _acquire(
        namespace, tmp_path, "84baffed", "cursor-84baffed",
...
def test_claim_displaced_recheck_sits_between_lock_and_process() -> None:
    ...
    recheck = source.index('_final_owner,_final_at,_final_revision=_current_claim_identity_fresh(cid)')

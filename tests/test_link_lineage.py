...
@pytest.mark.parametrize(
    ("title", "labels"),
    [
        ("[REREVIEW] Check amended source", ["parent-source01"]),
        ("Verify exact candidate", ["seat-seraph", "parent-source01"]),
    ],
)
def test_canonical_seraph_review_markers_are_not_sources(tmp_path, title, labels):
    _home(tmp_path, "source01", "review01")
    cards = [
        {
            "id": "source01",
    ...
    assert out["records"]["org/repo#7"]["source_card"] == "source01"
    assert out["records"]["org/repo#7"]["review_card_id"] == "review01"
...
def test_detailed_blocked_verdict_preserves_typed_terminal_lineage(tmp_path):
    _home(tmp_path, "source01", "review01")
    cards = _cards()
    cards[1]["links"] = {
        "verdict": "BLOCKED blocked_on=dependency referent=card:a1b2c3d4",
        "pr": "https://github.com/org/repo/pull/7",
        "commit": "a" * 40,
    }
    out = mod.reconcile(
    ...
    assert out["records"]["org/repo#7"]["review_verdict"] == "BLOCKED"
...
def test_unique_exact_source_binding_outranks_ambiguous_prose(tmp_path):
    head = "a" * 40
    _home(tmp_path, "source01", "legacy01", "review01")
    review = _linked_review("review01", "source01")
    review["links"].update({"pr": "https://github.com/org/repo/pull/17", "commit": head})
    cards = [
        _linked_source("source01", number=17, commit=head),
        {"id": "legacy01", "title": "Old discussion of PR #17", "labels": []},
        review,
    ]
    out = mod.reconcile(
        [
    ...
    assert out["records"]["org/repo#17"]["source_card"] == "source01"
    assert out["records"]["org/repo#17"]["review_card_id"] == "review01"

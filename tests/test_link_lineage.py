...
def _cards():
    return [
        ...
        {"id": "review01", "title": "Independent review PR #7", "labels": ["review", "parent-source01"], "status": "done", "links": {"verdict": "PASS"}},
    ]


def test_complete_emits_producer_contract(tmp_path):
    _home(tmp_path, "source01", "review01")
    out = mod.reconcile([{"repository": "org/repo", "number": 7, "headRefOid": "a" * 40, "baseRefOid": "b" * 40}], _cards(), tmp_path)
...
def test_incomplete_is_not_emitted_as_record(tmp_path):
    ...
    out = mod.reconcile([{"repository": "org/repo", "number": 7}], [{"id": "source01", "title": "Implement PR #7"}], tmp_path)
    assert out["records"] == {}
...
def test_pr_body_card_references_resolve_lineage_without_pr_number(tmp_path):
    ...
    cards = [
        ...
        {"id": "e5f6a7b8", "title": "Independent review", "labels": ["review", "parent-a1b2c3d4"], "status": "done", "links": {"verdict": "PASS"}},
    ]
    out = mod.reconcile(
        [{"repository": "org/repo", "number": 9, "body": "Source card a1b2c3d4; review e5f6a7b8", "headRefOid": "a" * 40, "baseRefOid": "b" * 40}],
        cards,
        tmp_path,
    )
...
def test_legacy_review_title_is_not_treated_as_source(tmp_path):
    ...
    cards = [
        ...
        {"id": "e5f6a7b8", "title": "[REVIEW] Check source", "labels": ["parent-a1b2c3d4"], "status": "done", "links": {"verdict": "PASS"}},
    ]
    out = mod.reconcile(
        [{"repository": "org/repo", "number": 10, "body": "a1b2c3d4", "headRefOid": "a" * 40, "baseRefOid": "b" * 40}],
        cards,
        tmp_path,
    )
    assert out["coverage"]["lineage-complete"] == 1
...
def test_pass_for_review_is_not_terminal_pass(tmp_path):
    ...
    cards = [
        {"id": "a1b2c3d4", "title": "Implement source", "labels": []},
        {"id": "e5f6a7b8", "title": "[REVIEW] Check source", "labels": ["review", "parent-a1b2c3d4"], "status": "done", "links": {"verdict": "PASS_FOR_REVIEW"}},
    ]
    out = mod.reconcile(
        [{"repository": "org/repo", "number": 11, "body": "a1b2c3d4", "headRefOid": "a" * 40, "baseRefOid": "b" * 40}],
        cards,
        tmp_path,
    )
    assert out["coverage"]["unresolved"] == 1
...
def test_terminal_fail_is_reconciled_but_not_a_pass(tmp_path):
    ...
    cards = [
        {"id": "a1b2c3d4", "title": "Implement source", "labels": []},
        {"id": "e5f6a7b8", "title": "[REVIEW] Check source", "labels": ["review", "parent-a1b2c3d4"], "status": "done", "links": {"verdict": "FAIL"}},
    ]
    out = mod.reconcile(
        [{"repository": "org/repo", "number": 12, "body": "a1b2c3d4", "headRefOid": "a" * 40, "baseRefOid": "b" * 40}],
        cards,
        tmp_path,
    )
    assert out["coverage"]["lineage-complete"] == 1
...
def test_review_required_source_label_is_not_reviewer(tmp_path):
    ...
    out = mod.reconcile(
        [{"repository": "org/repo", "number": 13, "body": "a1b2c3d4", "headRefOid": "a" * 40, "baseRefOid": "b" * 40}],
        cards,
        tmp_path,
    )
    assert out["coverage"]["lineage-complete"] == 1
...
def test_unique_terminal_review_wins_over_historical_nonterminal_review(tmp_path):
    ...
    cards = [
        ...
        {"id": "e5f6a7b8", "title": "[REVIEW] Historical review", "labels": ["parent-a1b2c3d4"], "status": "done", "links": {"verdict": "PASS_FOR_REVIEW"}},
        {"id": "e5f6a7b9", "title": "[REVIEW] Current review", "labels": ["parent-a1b2c3d4"], "status": "done", "links": {"verdict": "FAIL"}},
    ]
    out = mod.reconcile(
...
def test_exact_pr_and_head_binding_resolves_multiple_terminal_reviews(tmp_path):
    ...
    cards = [
        ...
        {"id": "e5f6a7b8", "title": "[REVIEW] Historical review", "labels": ["parent-a1b2c3d4"], "status": "done", "links": {"verdict": "PASS"}},
        {"id": "e5f6a7b9", "title": "[REVIEW] Current review", "labels": ["parent-a1b2c3d4"], "status": "done", "links": {"verdict": "FAIL", "pr": "https://github.com/org/repo/pull/15", "commit": head}},
    ]
    out = mod.reconcile(

...
def test_rejections(tmp_path):
    ...
    p = packet(tmp_path)
    assert admit_work_packet({**p, "repository": "other"}, repository="skcapstone").reason == AdmissionReason.WRONG_REPOSITORY
    assert admit_work_packet({**p, "candidate": {"path": "no", "sha256": "x"}}).reason == AdmissionReason.MISSING_CANDIDATE
    assert admit_work_packet({**p, "dependencies": ["gone"]}, dependencies={}).reason == AdmissionReason.MISSING_DEPENDENCY
    assert admit_work_packet({**p, "commands": ["pytest; rm -rf /"]}).reason == AdmissionReason.UNVERIFIABLE_COMMAND

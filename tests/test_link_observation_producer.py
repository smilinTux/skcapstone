...
def lineage(*, complete: bool = True) -> dict:
    record = {
        ...
        "mapping_evidence_sha256": _mapping_evidence(
            "smilinTux/skcapstone", 17, HEAD, BASE, "card-17", "generation-1", "review-17", REVISION, "PASS"
        ),
    }
    if not complete:
...
def test_jarvis_name_spoof_with_seraph_seat_is_blocked_before_feed_emission(tmp_path: Path) -> None:
    lineage_path = tmp_path / "lineage.json"
    invalid = lineage()

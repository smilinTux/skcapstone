...
**⚠️ coordination / ITIL / cards no longer live here.** `skcoord>=0.1.32` is a **hard
...
- The `0.1.32` floor is intentional. It retains the lifecycle and authoritative
...
checks:
  ...
  - name: skcoord is still a hard dep and coordination is still a shim over it
    run: grep -qxF '    "skcoord>=0.1.32",' pyproject.toml && grep -qxF 'import skcoord.coordination as _src' src/skcapstone/coordination.py && grep -qxF 'sys.modules[__name__] = _src' src/skcapstone/coordination.py
